...
function AdminSurvey({survey, toSubmit, addSurvey, user,surveyID, setSurveyID, answers, addAnswer, addResponse, getResponseOnClick}){
    let questions = survey.questions;
    const navigate = useNavigate();

    return(
        <div id={survey.id}  onClick={(e)=>{{getResponseOnClick && setSurveyID(e.target.id)}; {getResponseOnClick && navigate('/all_responses',{replace: false, state: {survey:survey}})}}}>
        <SurveyHeader survey={survey}/>
        <form className="survey-container" onSubmit={(e)=>{e.preventDefault()}}>
